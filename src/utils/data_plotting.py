...
def plot_gr_logs_2_wells_high_res(df: pd.DataFrame, df2: pd.DataFrame, title: str) -> None:
    """
    Plots gamma-ray (GR) logs from the provided DataFrame on the specified axes.
    ...
    Returns:
        None
    """
    fig, ax = plt.subplots(1, 1, figsize=(4, df['TDEP'].max()))
    ...
    # Plot the original GR logs (0 to 150 range) with blue color
    ax.scatter(x_0_150, y_0_150, label=f'GR Original', color='blue', s=3, zorder=3)

    # Plot the adjusted GR logs (150 to 300 range) with the same color and style
    ax.scatter(x_150_300, y_150_300, color='blue', s=3, zorder=3)

    # Plot the original GR logs (0 to 150 range) with blue color
    ax.scatter(x2_0_150, y2_0_150, label='GR Interpolado', color='salmon', s=0.5, zorder=2)

    # Plot the adjusted GR logs (150 to 300 range) with the same color and style
    ax.scatter(x2_150_300, y2_150_300, color='salmon', s=0.5, zorder=2)
